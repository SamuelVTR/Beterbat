...
.btn.active {
  ...
  ul.nav-pills {
    ...
    <?php 
    $plan=$_SESSION['plantoupdate'];
    if($_SESSION["user"]=="Administrateur"){include("NavBaradmin.php");} else {     include("NavBarCommercial.php"); }         ?>
...
<div class="container">
    <div class="form-group">
      ...
      <input style="width:33%;" name="nom_plan" value='<?php print($plan[0]["Nom_plan"]);?>'  class="form-control" id="nom_plan" type="text" required>
    </div>
    <div class="form-group">
      <label for="Description">Description:</label>
      <input style="width:33%;" name="Description" type="text" value='<?php print $plan[0]["Description"]?>' class="form-control" rows="5" id="Description" required>
    </div>


    <?php $type=explode("/",$plan[0]["Branchement"])?>
    <div class="form-group">
        <label  for="type">Type Maison</label>
        <select name="type" class="custom-select-sm">
           
            <option <?php if($type[0]=="f3"){print "selected='selected'";}?> value="f3">F3</option>
            <option <?php if($type[0]=="f4"){print "selected='selected'";}?> value="f4">F4</option>
            <option <?php if($type[0]=="f5"){print "selected='selected'";}?> value="f5">F5</option>
        </select>
    </div>
    <div class="form-group">
      <label for="Enveloppe">Prix enveloppe sanitaire:</label>
      <input style="width:33%;"value='<?php print $type[1]?>' name="Enveloppe" type="number" min="0" class="form-control" rows="5" id="Enveloppe">
    </div>
    <div class="form-group">
        <label  for="TarifHT">Tarif HT:</label>
        <input style="width:33%;"value= <?php print $plan[0]["prix_HT"]?>  name="TarifHT" type="number" step="0.01" class="form-control" id="TarifHT" required>
    </div>
    <?php $fondation=json_decode($plan[0]["cout_option_specifique"]);?>
Gros béton de fondation :
<input  style="width:25%;" value='<?php print $fondation[0]?>' name="fondation"  type="number" value="0"  step="0.01" class="form-control" min="0"  >
 fondation semelles fillante :
<input  style="width:25%;"  value='<?php print $fondation[1]?>' name="semelle_fillante"  type="number" value="0"  step="0.01" class="form-control" min="0"  >
 fondation radier:
<input  style="width:25%;" value='<?php print $fondation[2]?>'  name="radier"  type="number" value="0"  step="0.01" class="form-control" min="0"  >
  Vide sanitaire:
<input  style="width:25%;"  value='<?php print $fondation[3]?>' name="Videsanitaire"  type="number" value="0" step="0.01" class="form-control" min="0"  >

    <div class="form-group">
    Sélectionner fichier PDF:
<div class="custom-file">
    <input style="width:33%;" value='<?php print $plan[0]["pdf"]?>' name="myfile" type="file" class="custom-file-input" id="customFile">
